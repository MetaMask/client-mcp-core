...
final class RunnerTests: XCTestCase {
  ...
  private func findElement(app: XCUIApplication, text: String) -> XCUIElement? {
    // Use exact equality to avoid taps drifting to substring matches.
    // Driver passes the exact identifier/label/value resolved from a prior snapshot;
    // the runner must not pick a different element with overlapping text.
    let predicate = NSPredicate(format: "identifier == %@ OR label == %@ OR value == %@", text, text, text)
  ...
  /// Fills a text field by temporarily writing to the system pasteboard and tapping "Paste".
  /// Never leaves sensitive text on the pasteboard: the original contents are captured and
  /// restored via `defer` on every exit path (success, error, or missing menu).
