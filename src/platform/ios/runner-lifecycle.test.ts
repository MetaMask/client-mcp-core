...
describe('runner-lifecycle', () => {
  ...
  describe('stopRunner', () => {
    ...
    it('kills a runner that has not yet emitted its port', async () => {
      mockReaddir.mockResolvedValue(['Test.xctestrun']);
      const proc = createMockProcess();
      Object.defineProperty(proc, 'exitCode', { value: null, writable: true });
      Object.defineProperty(proc, 'signalCode', {
      ...
      await stopRunner('platform=iOS Simulator,id=AAA-111');

      expect(proc.kill).toHaveBeenCalled();
      await expect(portPromise).rejects.toThrowError(
        'Runner exited with code 0 before emitting port',
      );
    });
...
describe('registerCleanupHandlers – signal re-raise', () => {
  let killSpy: Mock;
  ...
  async function loadFreshModule(): Promise<{
    ...
    const mod = await import('./runner-lifecycle.js');
    return { registerCleanupHandlers: mod.registerCleanupHandlers };
  }

  it('re-raises SIGINT via process.kill after cleanup', async () => {
    const { registerCleanupHandlers } = await loadFreshModule();
  ...
  it('re-raises SIGTERM via process.kill after cleanup', async () => {
    const { registerCleanupHandlers } = await loadFreshModule();
    registerCleanupHandlers();
  ...
  it('is idempotent – second signal does not re-trigger cleanup', async () => {
    const { registerCleanupHandlers } = await loadFreshModule();
    registerCleanupHandlers();
    ...
    // Fire SIGTERM – shutdownStarted flag prevents a second cleanup
    sigtermListeners[0]();
