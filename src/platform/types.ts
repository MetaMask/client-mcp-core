...
export type IPlatformDriver = {
  ...
  /**
   * Get the configured application bundle ID for this session (iOS).
   *
   * Used by hermes_cdp to enforce strict appId matching against Metro's
   * /json discovery payload. If undefined, hermes_cdp fails closed
   * with MM_HERMES_TARGET_NOT_VERIFIED.
   *
   * Implementations on non-iOS platforms may omit this method.
   *
   * @returns The bundle ID (e.g., 'io.metamask.MetaMask') or undefined.
   */
  getAppId?(): string | undefined;

  /**
   * Get the session-scoped Metro inspector proxy port.
   *
   * When defined, this is the per-session port that was wired through
   * SessionLaunchInput.metroPort at consumer launch time. hermes_cdp
   * uses this as the fallback when its per-call metroPort input is
   * omitted (resolution chain: input.metroPort ?? driver.getMetroPort() ?? 8081).
   *
   * Implementations on non-iOS platforms may omit this method.
   *
   * @returns The Metro port (1-65535) or undefined.
   */
  getMetroPort?(): number | undefined;

  /**
   * Get the pinned Hermes logicalDeviceId for this session, if any.
   *
   * Once hermes_cdp successfully verifies a target, it pins that target's
   * reactNative.logicalDeviceId to the driver. Subsequent calls filter
   * Metro discovery by this pin to prevent target switching mid-session.
   *
   * Implementations on non-iOS platforms may omit this method.
   *
   * @returns The pinned logicalDeviceId or undefined (no pin yet).
   */
   ...
   * The pin lives for the lifetime of the driver instance (cleared on
   * session cleanup, since the driver is per-session).
   *
   * Implementations on non-iOS platforms may omit this method.
   *
