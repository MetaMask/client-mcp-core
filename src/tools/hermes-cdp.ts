...
export async function hermesCdpTool(
  ...
  if (context.platformDriver?.getPlatform() !== 'ios') {
  ...
  const { platformDriver } = context;
  const expectedAppId = platformDriver.getAppId?.();
  if (!expectedAppId) {
    return createToolError(
  ...
  const metroPort = input.metroPort ?? platformDriver.getMetroPort?.() ?? 8081;
  const pinnedDeviceId = platformDriver.getPinnedHermesDeviceId?.();
  ...
  try {
    ...
    return await executeVerifiedCdpCommand(
      ...
      input,
      pinnedDeviceId,
      platformDriver.setPinnedHermesDeviceId?.bind(platformDriver),
    );
  } catch (error) {
...
function selectHermesTarget(
  ...
  if (hasAmbiguousTarget(candidates)) {
    return {
      ...
      message: `Ambiguous Hermes target after pin+capability filtering. Candidates: ${candidates
        .map(
          (target) =>
