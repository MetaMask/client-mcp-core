...
describe('hermesCdpTool', () => {
  ...
  it('rejects cross-device candidates even when nativePageReloads differs', async () => {
    mockDiscovery([
      target({
        id: 'modern',
  ...
  it('prefers nativePageReloads as same-device tiebreak', async () => {
    mockDiscovery([
      target({
        id: 'stale-page',
  ...
  it('rejects iOS platform drivers without getAppId support', async () => {
    const context = {
      ...
      knowledgeStore: {},
      toolRegistry: new Map(),
      driver: { getPlatform: vi.fn().mockReturnValue('ios') },
    } as unknown as ToolContext;
