...
SAFE_ROOT="$HOME/.metamask-mobile-cli"
PACKAGE_OUTPUT_DIR="${IOS_RUNNER_DERIVED_DATA_PATH:-$SAFE_ROOT/ios-runner/derived}"
IOS_RUNNER_DEVICE_NAME="${IOS_RUNNER_DEVICE_NAME:-iPhone 16 Pro}"
CHECKSUM_FILE="$PACKAGE_OUTPUT_DIR/.source-checksum"
BUILD_TMP_DIR="$REPO_ROOT/.ios-runner-build-tmp"

assert_under_safe_root() {
    local target="$1"

    # Resolve canonical paths to prevent traversal via ".." or symlinks.
    local resolved_root resolved_target target_parent
    mkdir -p "$SAFE_ROOT"
    resolved_root="$(cd "$SAFE_ROOT" && pwd -P)"

    # Target may not exist yet — resolve its parent directory.
    target_parent="$(dirname "$target")"
    mkdir -p "$target_parent"
    resolved_target="$(cd "$target_parent" && pwd -P)/$(basename "$target")"

    case "$resolved_target" in
        "$resolved_root"|"$resolved_root"/)
            echo "❌ Refusing to delete the safe root itself: $target (resolves to $resolved_target)" >&2
            exit 1
            ;;
        "$resolved_root"/*)
            return 0
            ;;
        *)
            echo "❌ Refusing to rm -rf '$target' (resolves to '$resolved_target', not under '$resolved_root')" >&2
            exit 1
            ;;
    esac
}
...
xcodebuild build-for-testing \
    ...
    -destination "platform=iOS Simulator,name=$IOS_RUNNER_DEVICE_NAME" \
...
assert_under_safe_root "$PACKAGE_OUTPUT_DIR"
