...
| ------------- | --------------------------------------------------------------------------- | ---------------------------------------------- |
| **Mutating**  | click, type, navigate, launch, cleanup, build, clipboard, cdp, hermes_cdp, mock_network | Yes — `state` + `a11y` (compacted) + `testIds` |
...
The CLI supports iOS sessions via XCUITest + Hermes CDP. iOS requires a macOS host with Xcode 15+ installed.

#### `mm launch --platform ios`

Launches a MetaMask Mobile session in the iOS simulator.

```
mm launch --platform ios --device <udid> --app-bundle <path> [--metro-port <port>] [--reinstall] [--reset-app-data] [--allow-fox-code-mismatch] [--goal <text>] [--flow-tags <tags>]
```

| Flag                          | Description                                                                |
| ----------------------------- | -------------------------------------------------------------------------- |
| `--platform ios`              | Target the iOS simulator (required for mobile)                             |
| `--device <udid>`             | iOS simulator UDID (use `mm devices` to list)                              |
| `--app-bundle <path>`         | Path to the built `.app` bundle                                            |
| `--metro-port <port>`         | Metro inspector proxy port (default: 8081). Session-scoped.                |
| `--reinstall`                 | Reinstall the app bundle before launch                                     |
| `--reset-app-data`            | Clear app data before launch                                               |
| `--allow-fox-code-mismatch`   | Bypass the fox_code compatibility guard                                    |
| `--goal <text>`               | Tag the session with a goal for knowledge store                            |
| `--flow-tags <tags>`          | Comma-separated flow tags for cross-session knowledge                      |

**Platform support:** On iOS, interaction tools (`click`, `type`, `wait-for`, `get-text`), discovery tools (`describe-screen`, `accessibility-snapshot`, `list-testids`), `screenshot`, and `get-state` are supported. Browser-only tools (e.g., `navigate`, `switch-to-tab`, `close-tab`, `clipboard`, `cdp`, `mock-network`) return `MM_TOOL_NOT_SUPPORTED_ON_PLATFORM`.

#### `mm devices`

Lists available iOS simulator UDIDs.

```
mm devices
```

#### `mm hermes-cdp <method> [params-json]`

Sends a raw Chrome DevTools Protocol command to the active Hermes runtime (React Native) via Metro's inspector proxy. This is the iOS equivalent of `mm cdp`.

```bash
mm hermes-cdp Runtime.evaluate '{"expression":"1+1"}'
mm hermes-cdp Runtime.evaluate '{"expression":"global.__DEV__"}' --timeout 10000
mm hermes-cdp Runtime.getProperties '{"objectId":"1"}' --metro-port 8082
```
...
| `[params-json]` | Optional JSON object with method-specific parameters                             |
| `--timeout`     | Per-command timeout in ms (default: 30 000, max: 120 000)                        |
| `--metro-port`  | Metro port override. Optional if session was launched with `--metro-port`.        |

**Blocked methods**: `Runtime.terminateExecution`, `Inspector.detached`. Attempting a blocked method returns `MM_HERMES_CDP_BLOCKED`.

**Identity verification**: Every call enforces strict identity checks — appId match, HermesInternal probe, device pin, and WebSocket URL validation. If verification fails, the tool returns `MM_HERMES_TARGET_NOT_VERIFIED` rather than executing against a wrong target.

The tool is categorized as **mutating** — run `describe-screen` afterward to re-sync.
...
| `MM_TOOL_NOT_SUPPORTED_ON_PLATFORM` | Tool not supported on the active platform (e.g., iOS)                                                        | Use a supported tool for this platform; browser-only tools don't work on iOS                                                                                |
| `MM_IOS_ELEMENT_NOT_FOUND`       | iOS element not found within timeout                                                                            | Run `mm describe-screen` to get fresh refs; verify element exists                                                                                           |
| `MM_IOS_SNAPSHOT_FAILED`         | XCUITest snapshot command failed                                                                                | Runner may need recovery; retry or restart session                                                                                                          |
| `MM_IOS_EMPTY_SNAPSHOT`          | iOS discovery snapshot is empty after a rebind retry                                                            | App may still be rendering — wait briefly, then retry `mm describe-screen`                                                                                  |
| `MM_IOS_RUNNER_RECOVERING`       | XCUITest runner is recovering from a transient failure                                                          | Wait briefly and retry; the runner auto-recovers                                                                                                            |
| `MM_IOS_RUNNER_NOT_READY`        | XCUITest runner not yet ready                                                                                   | Wait for runner initialization to complete; retry after a few seconds                                                                                       |
| `MM_IOS_APP_IDENTITY_MISMATCH`   | App bundle ID mismatch between session and device                                                               | Verify `--app-bundle` matches the installed app; check consumer's `appBundleId` configuration                                                               |
| `MM_IOS_AX_DEVICE_NOT_FOUND`     | iOS AX snapshot couldn't bind to the session simulator (UDID mismatch or no matching window)                    | Verify only one simulator with the session's device name is running; check macOS Accessibility permission for the AXSnapshot binary; see README iOS section |
| `MM_IOS_AX_PERMISSION_REQUIRED`  | macOS Accessibility permission not granted to the AXSnapshot binary                                             | Grant Accessibility access: System Settings > Privacy & Security > Accessibility                                                                            |
| `MM_IOS_AX_BINARY_MISSING`       | AXSnapshot binary unavailable or could not be built                                                             | Verify Xcode and Swift toolchain installed; check `~/.metamask-mobile-cli/` cache                                                                           |
| `MM_IOS_AX_SNAPSHOT_FAILED`      | AXSnapshot binary returned a runtime error                                                                      | Check simulator state; retry `mm describe-screen`                                                                                                           |
| `MM_HERMES_CONNECTION_FAILED`    | WebSocket connection to Hermes runtime failed                                                                   | Verify Metro is running on the expected port; check network connectivity                                                                                    |
| `MM_HERMES_TARGET_NOT_FOUND`     | No Hermes target matches the session `appId` (or the pinned device disappeared)                                 | Verify Metro is running on the expected port; check that the app is bound to the debugger                                                                   |
| `MM_HERMES_TARGET_NOT_VERIFIED`  | Hermes target failed identity verification (non-Hermes target, or multi-device ambiguity without a session pin) | Confirm only one simulator is running MetaMask; verify the consumer's `appBundleId` matches the build variant; see README Hermes CDP Identity Verification  |
| `MM_HERMES_UNSAFE_TARGET`        | WebSocket URL failed safety validation                                                                          | Ensure Metro is running on localhost; do not use remote or proxied Metro instances                                                                           |
| `MM_HERMES_DEVICE_PIN_MISMATCH`  | Pinned device differs from discovered target                                                                    | Session's device pin changed (app reload/simulator wipe); restart session                                                                                   |
| `MM_HERMES_CDP_BLOCKED`          | Hermes CDP method blocked (destructive)                                                                         | Use a different CDP method; see blocked list (`Runtime.terminateExecution`, `Inspector.detached`)                                                            |
| `MM_HERMES_CDP_FAILED`           | Hermes CDP command execution failed                                                                             | Check method name/params; retry or increase timeout                                                                                                         |
